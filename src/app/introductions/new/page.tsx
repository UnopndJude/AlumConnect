...
async function handleSubmit(data: IntroductionFormData) {
  ...
  const response = await fetch(
    `${process.env.NEXT_PUBLIC_BASE_URL || "http://localhost:3000"}/api/introductions`,
    {
      method: "POST",
      headers: {
...
export default async function NewIntroductionPage() {
  ...
  const existingIntroduction = await introductionRepository.findByUserId(
      ...
      <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            동문들에게 자신을 소개해보세요. 필수 정보만 입력하시거나, 더 많은
            정보를 공유하여 동문들과 더 깊은 네트워킹을 할 수 있습니다.
          </p>
        </div>
